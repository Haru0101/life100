...
const Home: NextPage = () => {
  ...
  const listItems = items?.map((item, index) => {
    return <li className='w-1/2 flex items-center' key={index}><input type='checkbox' checked={item.isDone} onChange={e => completeItem(index, e.target.checked)} name='' id='' /><span className={item.isDone ? 'line-through' : ''}>{item.content}</span><button onClick={() => deleteItem(index)}><svg xmlns="http://www.w3.org/2000/svg" className="ml-1 h-4 w-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
